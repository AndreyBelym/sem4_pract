...
\begin{document}
\selectlanguage{russian}
\maketitle {МЕТОДЫ ОПТИМИЗАЦИИ}{4}
\ssec{Аннотация}
\normalfont
В данной работе разобрана задача оптимизации функции от двух переменных методом наискорейшего спуска.

В работе написана программа на языке Python, описаны функции, приведён тестовый пример.

Пояснительная записка к курсовой работе состоит из 17 страницы, и включает в себя 3 рисунка. Содержит следующие пункты: введение, постановка задачи, разработка алгоритма, текст программы, тестовая задача, инструкция пользователю, заключение и список используемой литературы.
\end{document}
