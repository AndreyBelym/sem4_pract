...
\begin{document}
\selectlanguage{russian}
\maketitle {АЛГОРИТМ ЙЕНА}{5}
\ssec{Аннотация}
\normalfont
В работе рассматривается задача поиска нескольких кратчайших путей на заданном графе. Задача решается с помощью алгоритмов Йена и Дейкстры.

В работе разработана схема алгоритма, написана программа на языке Python, приведён тестовый пример.

Пояснительная записка к курсовой работе состоит из 23 страниц, и включает в себя 5 рисунков и 2 таблицы. Содержит следующие пункты: введение, постановка задачи, разработка алгоритма, текст программы, тестовая задача, инструкция пользователю, заключение и список используемой литературы.
\end{document}
